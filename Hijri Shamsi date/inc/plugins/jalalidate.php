...
function jalalidate_info()
{
    return array(
        "name"          => "تاریخ هجری شمسی",
        "description"   => "تبدیل تاریخ سیستم انجمن ساز مای بی بی به هجری شمسی",
...
function jalalidate_my_date($date)
{
	global $mybb;
	...
	if($dateformat=="m-d-Y")
	{
		...
		if(!empty($timest) && is_numeric($timest))
		{
		$date=jalali_date($dateformat,$timest);
		}

		}	
		else if(preg_match("#[a-zA-Z]+ [0-9]{4}#",$date))
		{
		$date=jalali_date("d-m-Y",strtotime($date));
...
function jalali_date($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
...
$plugins->add_hook("my_date", "jalalidate_my_date");
  ...
  if($m===''){
   ...
   if($s===''){
    ...
    if($jm===''){
     ...
     $jdate=explode('_',jalali_date('Y_j','','',$timezone,'en'));
     if($jd===''){
...
function jgetdate($timestamp='',$none='',$timezone='Asia/Tehran',$tn='fa'){
 $ts=($timestamp==='')?time():tr_num($timestamp);
 $jdate=explode('_',jalali_date('F_G_i_j_l_n_s_w_Y_z',$ts,'',$timezone,$tn));
 return array(
